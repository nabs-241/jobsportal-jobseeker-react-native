...
interface CompanyCvPackagesProps {
  onBack: () => void;
...
const CompanyCvPackages: React.FC<CompanyCvPackagesProps> = ({
  onBack,
  ...
  const loadData = async () => {
    try {
      const res = await companyService.getCvPackages();
      const data = (res as any)?.message ?? (res as any)?.data ?? res;
      setPackages(Array.isArray(data?.packages) ? data.packages : []);
      setCurrentPackage(data?.success_package ?? null);
  ...
  const handleBuyNow = (pkg: any) => {
    ...
    onNavigateToPackagePurchase(Number(id), isFree, 'cv');
  };

  const formatPrice = (pkg: any) => {
    const currency = (pkg.currency ?? pkg.currency_code ?? 'USD').toString().toUpperCase();
    const price = pkg.package_price ?? pkg.price ?? 0;
    return `${currency} ${price}`;
  };

  const cvViews = (pkg: any) => pkg.package_num_listings ?? pkg.cvs_quota ?? pkg.num_listings;
  const cvDays = (pkg: any) => pkg.package_num_days ?? pkg.num_days ?? pkg.duration_days;

  return (
    <View style={styles.container}>
      <Header
        title={t('company_view_cv_packages')}
        onBack={onBack}
        showBack
        ...
        >
          {!packagesEnabled ? (
            ...
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyText}>{t('no_cv_packages')}</Text>
            </View>
          ) : (
            packages.map((pkg) => {
              const views = cvViews(pkg);
              const days = cvDays(pkg);
              return (
                <View key={pkg.id} style={styles.packageCard}>
                  <View style={styles.cardHeader}>
                    <View style={styles.cardIcon}>
                      <MaterialIcons name="search" size={28} color="#17D27C" />
                    </View>
                  ...
                  <View style={styles.featuresList}>
                    {views != null && (
                      <View style={styles.featureRow}>
                        ...
                        <Text style={styles.featureText}>{t('applicant_cv_views')} {views}</Text>
                      </View>
                    )}
                    {days != null && (
                      <View style={styles.featureRow}>
                        <MaterialIcons name="check-circle" size={18} color="#17D27C" style={styles.featureIcon} />
                        <Text style={styles.featureText}>{t('cv_view_access_days')} {days} {t('days')}</Text>
                      </View>
                    )}
                    <View style={styles.featureRow}>
                      <MaterialIcons name="check-circle" size={18} color="#17D27C" style={styles.featureIcon} />
                      <Text style={styles.featureText}>{t('premium_support')}</Text>
                    </View>
                  </View>
...
export default CompanyCvPackages;
